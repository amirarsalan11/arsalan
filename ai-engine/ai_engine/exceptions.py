...
class LightingAdjustmentError(PipelineStageError):
    """Raised by the Lighting stage when PerspectiveTransformResult's
    warped image arrays are malformed (wrong shape/dtype, empty), or
    when a caller-supplied target luminance is out of the valid
    [0, 255] range or non-finite.

    Distinct from PerspectiveTransformError: this stage receives an
    ALREADY-PRODUCED PerspectiveTransformResult and validates it
    defensively, the same way Perspective validates the FloorGeometry
    it receives — each stage owns its own dedicated exception rather
